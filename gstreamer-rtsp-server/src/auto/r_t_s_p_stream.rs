...
impl<O: IsA<RTSPStream> + IsA<glib::object::Object>> RTSPStreamExt for O {
    ...
    fn handle_keymgmt(&self, keymgmt: &str) -> bool {
        unsafe {
            from_glib(ffi::gst_rtsp_stream_handle_keymgmt(self.to_glib_none().0, keymgmt.to_glib_none().0))
        }
    }
    ...
    fn request_aux_receiver(&self, sessid: u32) -> Option<gst::Element> {
        unsafe {
            from_glib_full(ffi::gst_rtsp_stream_request_aux_receiver(self.to_glib_none().0, sessid))
        }
    }
